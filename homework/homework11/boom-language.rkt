...
;; (with <var> = <exp> do <exp>)
(define boom/with/keyword car)
(define boom/with/variable cadr)
(define boom/with/equals caddr)
(define boom/with/var-value cadddr)
(define boom/with/do (lambda (exp) (car (cdr (cdddr exp)))))
(define boom/with/exp (lambda (exp) (car (cdr (cdr (cdddr exp))))))
...
(define boom/with?
  (lambda (exp)
    (and (not (null? exp))
         (eq? (length exp) 6)
         (eq? (boom/with/keyword exp) 'with)
         (boom/varref? (boom/with/variable exp))
         (eq? (boom/with/equals exp) '=)
         (boom/exp? (boom/with/var-value exp))
         (eq? (boom/with/do exp) 'do)
...
(display "boom/with/* accessor tests\n")
(define with-exp '(with var = exp1 do exp2))
(eq? (boom/with/keyword with-exp) 'with)
(eq? (boom/with/variable with-exp) 'var)
(eq? (boom/with/equals with-exp) '=)
(eq? (boom/with/var-value with-exp) 'exp1)
(eq? (boom/with/do with-exp) 'do)
(eq? (boom/with/exp with-exp) 'exp2)

(display "boom/with? tests\n")
(boom/with? '(with x = 2 do (15 + x)))
(not (boom/with? '()))
(not (boom/with? '(1 / 2)))

(display "boom/with/eval tests\n")
